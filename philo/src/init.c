...
/*   Updated: 2025/04/02 18:33:24 by fvizcaya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_set_fork_numbers(t_philo *philo)
{
	...
	if (philo->id % 2 == 0)
	{
		philo->l_fork = &philo->dinner->forks[philo->id];
		philo->r_fork = &philo->dinner->forks[(philo->id + 1) % num_philos];
	}
	else
	{
		philo->l_fork = &philo->dinner->forks[(philo->id + 1) % num_philos];
		philo->r_fork = &philo->dinner->forks[philo->id];
	}
}
...
int	ft_init(t_dinner *dinner)
{
	...
	dinner->init_time = ft_get_current_time();
	while (i < dinner->args->num_philos)
